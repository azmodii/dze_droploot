...
_tempContainer = createVehicle ["weaponHolder", _shinyNewPOS, [], 0, "can_collide"];
...
{	
	if (_exDebugEnabled) then { 
	diag_log format["VEHICLE LOOT EX DEBUG: Adding Backpack : %1 | At : %2",_x, getPosATL _tempContainer];  
	}; 
	_count = _bCounts select (_backpacks find _x);
	_bp = createVehicle [_x, _shinyNewPOS, [], 2, "can_collide"];
	if (_waitTime > 0) then { } else {
		if (_debugEnabled) then { 
			diag_log format["VEHICLE LOOT: Starting permaloot paramater on : %1",_bp];  
		};
		_bp setVariable["permaLoot", true];
	};
} forEach _backpacks;
